...
*/
    ...
    class dataBase{
        ...
        public function dycData($arr){
            $RSA = new RSA(); 
            $keys = $RSA->generate_keys ('9990454949', '9990450271', 0); 
            $res=[];
            for($i = 0;$i<sizeof($arr);$i++){
            $res[$i] = $RSA->decrypt($arr[$i], $keys[2], $keys[0]);
            }
            return $res; 
            }
        public function select($select = "*", $whereIn = array(), $whereEq = array(), $get = true){
            ...
            $incarr = $this->incData($whereEq);
            $fin = $this->mkInsertArray($whereIn, $incarr); //set values in [key => value] array to execute it
            if($get){
            $data = $this->executeQuery(false, true, $fin);
            $res = []; 
            if (!is_null($data))
            if(sizeof($data)>0)
            for($i=0;$i<sizeof($data);$i++){
            $res[$i]=$this->dycData($data[$i]);
            }
            }//if we pass parameter get true => return data
            }//end of mkQuery
// XXXXXXXXXXXXXXXXXXXXXXX
        ...
        public function incData($vals){
            $RSA = new RSA();
            $keys = $RSA->generate_keys ('9990454949', '9990450271', 0);
            $result=[];
            if(sizeof($vals)>0)
                for($i=0;$i<sizeof($vals);$i++){
                    $result[$i]=$RSA->encrypt ($vals[$i], $keys[1], $keys[0], 5);
                }
                return $result;
        }
        public function insert($cols, $vals){
            $valss = $this->incData($vals);
            ...
            $all = $this->mkInsertArray($cols, $valss);// mk insert array as shown
...
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXx
        public function mk(){
                ...
                $Make[] = 'CREATE TABLE wallet(
                    ...
                    activeBalance int(11) DEFAULT 0,
